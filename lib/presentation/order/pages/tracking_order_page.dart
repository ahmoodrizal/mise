...
class _TrackingOrderPageState extends State<TrackingOrderPage> {
  ...
  @override
  void initState() {
    context.read<OrderDetailBloc>().add(OrderDetailEvent.getOrderDetail(widget.orderId));
    super.initState();
  }
                ...
                children: [
                  ListView.separated(
                    ...
                    itemBuilder: (context, index) => ProductTile(
                      data: orderDetail.order!.orderItems![index],
                    ),
                  ...
                  Button.outlined(
                    onPressed: () {
                      if (orderDetail.order!.status == 'on_delivery') {
                        context.pushNamed(
                          RouteConstants.shippingDetail,
                          pathParameters: PathParameters().toMap(),
                          extra: orderDetail.order!.shippingResi!.toString(),
                        );
                      }
                    },
                    label: orderDetail.order!.status == 'on_delivery' ? 'Detail pelacakan pengiriman' : 'Pesanan dalam proses',
                  ),
                  ...
                  Text(
                    orderDetail.order!.address!.fullAddress!,
                  ...
                  Text(
                    orderDetail.order!.address!.name!,
                    style: const TextStyle(
                      fontSize: 16,
                    ),
                  ),
