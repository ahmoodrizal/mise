...
import 'package:mise/core/core.dart';
...
class _OrderDetailPageState extends State<OrderDetailPage> {
  @override
  void initState() {
    context.read<ShippingCostBloc>().add(
          const ShippingCostEvent.getShippingCost(
            origin: '317',
            destination: '514',
            weight: 1000,
            courier: 'jne',
              ...
              BlocBuilder<CheckoutBloc, CheckoutState>(
                builder: (context, state) {
                  final shippingCost = state.maybeWhen(
                    orElse: () => 0,
                    loaded: (_, __, ___, ____, shippingCost, _____) => shippingCost,
                  );
                  return Text(
                      ...
                      loaded: (data) {
                        return ListView.separated(
                          ...
                          itemBuilder: (context, index) {
                            final item = data.rajaongkir!.results![0].costs![index];
                            return ListTile(
                              contentPadding: EdgeInsets.zero,
                              onTap: () {
                                context.read<CheckoutBloc>().add(
                                      CheckoutEvent.storeShippingService(
                                        'jne',
                                        item.cost![0].value!,
                                      ),
                                    );
                              ...
                              title: Text('${item.service} - ${item.description} (${item.cost![0].value})'),
                              subtitle: Text('Estimasi tiba ${item.cost![0].etd} hari'),
                            );
                          },
                          separatorBuilder: (context, index) => const Divider(color: AppColors.stroke),
                          itemCount: data.rajaongkir!.results![0].costs!.length,
                        );
                      },
                    );
                  },
